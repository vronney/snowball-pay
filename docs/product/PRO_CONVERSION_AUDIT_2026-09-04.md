...
- Stripe customers: 5. Subscriptions: 3 (2 active, 1 canceled: C1 took the old 7-day card trial, the card then failed and Stripe closed the subscription (corrected 2026-09-05, see the Stripe check below), and C1 kept using Free for months as one of the most active accounts).
...
- **Correction to C1:** the canceled subscription ended on Stripe's `payment_failed` reason after the card trial, not a voluntary cancel before the first charge. C1 tried to pay and the card failed. That is a recoverable customer, not a churned one. Lever: a personal note offering to reactivate at the grandfathered price. Dates and amounts live in the private audit record.
...
1. **The 393 `calculator_form_blocked` events were not people.** 384 of them came from two automated sessions on a single day in early August, both arriving through a search-partner referral and each firing blocked-field events several times a second with rage clicks. Six real visitors triggered 1 to 3 events each, all on an empty balance, mostly on mobile. The validation copy is not a trap; drop this item.
2. **"Reset sample numbers" is the most-tapped button on the mobile calculator, and it does nothing on first load.** 66 taps across 9 mobile sessions, most of them from a single session that rage-clicked it; 6 taps were flagged dead. It is the only button in the hero, styled as a pill under the headline, and on a fresh page the data already is the sample, so tapping it changes nothing. Visitors read it as "start". Fix: remove it from the hero; show a small "Reset to sample" text link inside the debts card only after the user has edited something (`isSampleData === false`), and let the hero button focus the first balance field instead. `PublicCalculator.tsx`
