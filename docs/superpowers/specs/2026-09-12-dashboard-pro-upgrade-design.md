...
| `rateWatch.ts` | `computeRateWatch(debts)` | Negotiable cards (`isNegotiableCard`) with APR above target. Returns `{ cards, annualEstimate }` = Σ `estimateAnnualSavingsExact`, unrounded estimate (same formula as the APR card), floored for display. |
