...
- Rate limiting in middleware (60 req/IP/60s, returns 429)
- Scanner/bot path blocking in middleware (WordPress, xmlrpc.php probes → 404)
- CORS restricted to `APP_BASE_URL` (falls back to request origin in dev)
- CORS preflight handled for all `/api/*` routes
- SQL injection prevention (Prisma parameterizes all queries)
- Database encryption at rest (Neon AES-256)

🔄 **To Implement**:
- CSRF token validation
- XSS protection in inputs (CSP headers)
- Regular security audits
- Dependency vulnerability scanning (`npm audit` in CI)
- Persistent rate limiting across edge nodes (Upstash Redis)
...
- Logging sensitive data (PII, session tokens)
- Widening CORS beyond `APP_BASE_URL` in production
