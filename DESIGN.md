...
| 2026-09-20 | Calculator text contrast pass | Lighthouse on `/calculator` (2026-09-20) found 23 AA contrast failures, including the two numbers that make the Pro case (`vs Minimums` at 2.17:1, `Total Interest` at 2.05:1). Text-on-light swaps: green figures use `success-text` #15803d (also the chart's actual-balance line and "Plan paydown" pill); amber text `#f59e0b`/`#d97706` → `#b45309` (5.9:1); red cash-flow deductions `#f87171` → `#b91c1c` (6.0:1); the `#94a3b8` gray on eyebrows, slider bounds, CTA helper and footer links → `--muted` #64748b (4.8:1). `#f59e0b` stays for fills and badges. The consent banner also now waits for the first scroll, tap or keypress (8 s fallback) instead of covering the hero on load. |
