...
export default function FirstWinEmail({ userName = 'there', debtFreeDate, totalInterestSaved, debtCount, monthlyPayment }: FirstWinEmailProps) {
  return (
    <EmailLayout
      headerGradient="linear-gradient(135deg, #065f46, #059669)"
      headerLabel="Your plan is live 🎉"
    ...
    >
      ...
      <Section style={{ padding: '28px 40px 36px' }}>
        ...
        <Text style={{ fontSize: '15px', color: '#475569', lineHeight: '1.7', margin: '0 0 24px' }}>
          Your payoff plan across <strong>{debtCount} {debtCount === 1 ? 'debt' : 'debts'}</strong> is set. With <strong>{fmt(monthlyPayment)}/month</strong> you&apos;re on track to be completely debt-free by <strong>{debtFreeDate}</strong>.
        </Text>
        <Section style={{ background: '#f8fafc', borderRadius: '12px', padding: '16px 20px', border: '1px solid rgba(15,23,42,0.08)', marginBottom: '28px' }}>
          <Text style={{ fontSize: '13px', fontWeight: 700, color: '#334155', margin: '0 0 4px' }}>Your next move</Text>
          <Text style={{ fontSize: '14px', color: '#64748b', margin: 0, lineHeight: '1.6' }}>
            Bookmark your dashboard and check in each month after you make a payment. Logging your real balance keeps your plan accurate.
          </Text>
        </Section>
        <Button href={`${BASE}/dashboard`} style={{ background: '#059669', color: '#ffffff', borderRadius: '10px', padding: '14px 28px', fontSize: '15px', fontWeight: 700, textDecoration: 'none', display: 'inline-block' }}>
          View My Dashboard →
        </Button>
...
FirstWinEmail.PreviewProps = {
  debtFreeDate: 'March 2027',
  totalInterestSaved: 4820,
  debtCount: 3,
  monthlyPayment: 640,
} satisfies Parameters<typeof FirstWinEmail>[0];
