...
export default function IncompleteSetupEmail({ userName = 'there', hasDebts, hasIncome }: IncompleteSetupEmailProps) {
    ...
    >
      ...
      <Section style={{ padding: '24px 40px 36px' }}>
        ...
        <Text style={{ fontSize: '15px', color: '#475569', lineHeight: '1.7', margin: '0 0 24px' }}>
          You still need to add <strong>{missing}</strong> before we can show you your exact debt-free date and total interest savings.
        </Text>
...
IncompleteSetupEmail.PreviewProps = {
  hasDebts: true,
  hasIncome: false,
} satisfies Parameters<typeof IncompleteSetupEmail>[0];
