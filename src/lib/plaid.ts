...
/**
 * Index rows by their fallback key, keeping ONLY keys that exactly one row
 * claims.
 *
 * plaidFallbackMatchKey is deliberately not unique — two cards at one bank can
 * share a mask and a category — so a key claimed by two rows identifies
 * neither. Dropping it makes the lookup miss, which falls through to creating a
 * new debt: the behaviour that existed before fallback matching, and strictly
 * safer than re-linking an incoming account onto an arbitrarily chosen row and
 * overwriting its balance and Plaid linkage.
 *
 * Callers must apply this on BOTH sides — the stored debts and the incoming
 * Plaid accounts — since either can contain the collision.
 */
export function indexByUniqueFallbackKey<T>(
  rows: readonly T[],
