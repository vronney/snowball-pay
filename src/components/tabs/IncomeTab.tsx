...
export default function IncomeTab({
  ...
  const [formData, setFormData] = useState({
    monthlyTakeHome:
      income?.monthlyTakeHome != null ? String(income.monthlyTakeHome) : "",
    essentialExpenses:
      income?.essentialExpenses != null ? String(income.essentialExpenses) : "",
  });
    ...
    <section id="section-income">
      ...
      >
        ...
        <form onSubmit={handleIncomeSubmit} className="space-y-4">
          <div>
            <label className="text-xs opacity-60 mb-1 flex items-center gap-1">
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, monthlyTakeHome: e.target.value })
              }
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, essentialExpenses: e.target.value })
              }
              className="input-field"
            />
            <p className="text-xs opacity-40 mt-1">
              Rent, utilities, groceries, insurance — excluding debt payments
            </p>
          </div>
          ...
          {takeHome > 0 && (
            ...
            >
              ...
              >
                {(() => {
                  const pctExp = Math.min(
                    100,
                    (totalEssential / takeHome) * 100,
                  );
                  const pctMin = Math.min(
                    100 - pctExp,
                    (totalMinPayments / takeHome) * 100,
                  );
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-xs">
                ...
                {cancelledTotal > 0 && (
                  <span style={{ color: "#059669" }}>
                    ✂ {formatCurrency(cancelledTotal)}/mo targeted to cut
                  </span>
                )}
              </div>
            </div>
          ...
          {saveIncome.isSuccess && (
            <p className="text-emerald-400 text-xs">Budget saved!</p>
          )}
          {saveIncome.isError && (
            <p className="text-red-400 text-xs">Error saving budget</p>
          )}
        </form>
      </div>
        ...
        >
          ...
          <ChevronDown
            ...
            style={{
              ...
              transform: recurringOpen ? "rotate(180deg)" : "rotate(0deg)",
            }}
          />
        </button>
        ...
        {recurringOpen && (
          <div className="px-5 pb-5">
            <p className="text-xs opacity-60 mb-4">
              Track subscriptions, utilities, and other monthly recurring costs
            </p>
                    ...
                    >
                      <button
                        ...
                        style={{
                          ...
                          fontSize: "10px",
                          fontWeight: 700,
                          border: `1px solid ${isCancelled ? "rgba(16,185,129,0.3)" : "rgba(239,68,68,0.2)"}`,
                          background: isCancelled
                            ? "rgba(16,185,129,0.08)"
                            : "rgba(239,68,68,0.06)",
                          color: isCancelled ? "#059669" : "#dc2626",
                          cursor: "pointer",
                        }}
                      >
                        {isCancelled ? "↩ Keep" : "✂ Cut?"}
                      </button>
                      <button
