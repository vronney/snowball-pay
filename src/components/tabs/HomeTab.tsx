    ...
    >
      ...
      <div className="flex justify-around mt-1">
        <div className="text-center">
          <p className="text-xs" style={{ color: '#64748b' }}>Interest</p>
          <p className="font-bold text-base" style={{ color: '#f59e0b' }}>{interestPct}%</p>
        </div>
        <div className="text-center">
          <p className="text-xs" style={{ color: '#64748b' }}>Principal</p>
          <p className="font-bold text-base" style={{ color: '#2563eb' }}>{principalPct}%</p>
        </div>
      </div>
    </div>
  );
...
function DebtProgressChart({
  ...
  return (
    ...
    >
      ...
      <div className="h-56">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={balances} margin={{ top: 4, right: 10, left: 4, bottom: 4 }}>
            <defs>
    ...
    >
      <div className="flex items-center gap-2">
        <div
          ...
          style={{ width: 32, height: 32, background: iconBg, color: iconColor, flexShrink: 0 }}
        >
          {icon}
        </div>
...
function ActionButton({
  ...
  return (
    <button
      ...
      style={{ background: bg, border: `1px solid ${border}`, cursor: 'pointer', fontFamily: 'inherit' }}
    >
...
export default function HomeTab({
  ...
  const payoffResult = useMemo(() => {
    ...
    const targetAccel =
      income.accelerationAmount !== null && income.accelerationAmount !== undefined
  ...
  const monthlyMin = useMemo(() => debts.reduce((s, d) => s + d.minimumPayment, 0), [debts]);
  ...
  const topAlert = useMemo(
    () => notifications.find((n) => n.type === 'urgent' || n.type === 'warning') ?? notifications[0] ?? null,
    [notifications],
  );

  const payoffMethod = income?.payoffMethod ?? 'snowball';
  const methodLabel =
    payoffMethod === 'avalanche' ? 'Avalanche' : payoffMethod === 'custom' ? 'Custom' : 'Snowball';
    ...
    <section className="space-y-5">
      ...
      <div>
        <h1 className="text-2xl font-bold" style={{ color: '#0f172a' }}>
          {firstName ? `Hello ${firstName}, Let's Crush Your Debt!` : "Let's Crush Your Debt!"}
        </h1>
      ...
      >
        ...
        {payoffResult && (
          <>
            <div style={{ height: 40, width: 1, background: 'rgba(255,255,255,0.2)' }} className="hidden sm:block" />
            <div>
              <p className="text-xs font-semibold uppercase tracking-widest" style={{ color: 'rgba(255,255,255,0.7)' }}>
                Debt-Free
              </p>
              <p className="text-xl font-bold mt-1" style={{ color: '#ffffff' }}>{debtFreeLabel}</p>
            </div>
            <div style={{ height: 40, width: 1, background: 'rgba(255,255,255,0.2)' }} className="hidden sm:block" />
            <div>
              <p className="text-xs font-semibold uppercase tracking-widest" style={{ color: 'rgba(255,255,255,0.7)' }}>
              ...
              <p className="text-xl font-bold mt-1" style={{ color: '#ffffff' }}>{monthlyPayment}</p>
            </div>
            <div style={{ height: 40, width: 1, background: 'rgba(255,255,255,0.2)' }} className="hidden sm:block" />
            <div>
              <p className="text-xs font-semibold uppercase tracking-widest" style={{ color: 'rgba(255,255,255,0.7)' }}>
              ...
              <p className="text-xl font-bold mt-1" style={{ color: '#ffffff' }}>{methodLabel}</p>
            </div>
          </>
        )}
      </div>
      ...
      {noData ? (
        ...
        >
          <CreditCard size={36} style={{ color: '#2563eb', margin: '0 auto 12px' }} />
          <p className="font-semibold text-base mb-2" style={{ color: '#0f172a' }}>
            Add your debts to get started
          </p>
          <p className="text-sm mb-4" style={{ color: '#64748b' }}>
            Once you add debts and set up your income, your full payoff plan will appear here.
          </p>
          <button
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              {payoffResult && payoffResult.totalInterestPaid > 0 ? (
                <>
                  ...
                  <button
                    ...
                    style={{ color: '#8b5cf6', background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontFamily: 'inherit', fontSize: 'inherit' }}
                  >
            ...
            >
              <div className="space-y-1">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold" style={{ color: '#0f172a' }}>{monthlyPayment}</span>
                </div>
                <div className="flex justify-between">
                  <span style={{ color: '#64748b' }}>Payoff Strategy</span>
                  <span className="font-semibold" style={{ color: '#0f172a' }}>{methodLabel} Method</span>
                </div>
                {payoffResult && (
                  <div className="flex justify-between">
            ...
            >
              {topAlert ? (
                <div>
                  <p className="font-medium text-xs mb-1" style={{ color: '#0f172a' }}>{topAlert.title}</p>
                  <p className="text-xs leading-relaxed" style={{ color: '#64748b' }}>{topAlert.body}</p>
                </div>
              ) : (
                <p className="text-xs" style={{ color: '#64748b' }}>
                  No urgent alerts right now. Stay on track and keep making payments!
                </p>
              )}
            </InfoCard>
          </div>
