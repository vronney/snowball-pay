...
type ExtractedResult =
  | ExtractedDebtResult
  | ExtractedIncomeResult
  | ExtractedStatementResult;
...
const FILE_TYPE_OPTIONS: FileTypeOption[] = [
  {
    ...
    description:
      "Credit card, loan, or mortgage statement — extracts balance, APR, and minimum payment.",
  ...
  },
  {
    ...
    description:
      "Pay stub, W2, or offer letter — extracts your monthly take-home pay.",
  ...
  },
  {
    ...
    description:
      "Bank or card statement — detects recurring charges to help trim your expenses.",
...
function DropZone({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      ...
      >
        Your document is processed locally for text extraction and then
        discarded. Only the extracted data (balances, amounts) is stored — never
        the original file.
      </div>
...
function DebtReview({
  ...
  const confColor =
    conf >= 0.7 ? "#10b981" : conf >= 0.5 ? "#f59e0b" : "#ef4444";
  const confLabel =
    conf >= 0.7
      ? "High confidence"
      : conf >= 0.5
        ? "Medium confidence"
        : "Low — please review carefully";
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      ...
      >
        ...
        />
        <span style={{ fontSize: "12px", color: "#64748b" }}>
          {confLabel} — edit any field before saving.
        </span>
      </div>
        ...
        <FieldRow
          ...
          value={
            <input
              ...
              value={item.balance || ""}
              onChange={(e) =>
                onChange({ ...item, balance: parseFloat(e.target.value) || 0 })
              }
            />
          }
        />
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      <p style={{ fontSize: "13px", color: "#64748b", margin: 0 }}>
        Review the extracted data. Changing pay frequency will recalculate the
        monthly amount automatically.
      </p>
      <div
        style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}
      >
        <FieldRow
          ...
          value={
            <select
              ...
              onChange={(e) =>
                handleFrequencyChange(e.target.value as IncomeFreq)
              }
            >
      ...
      {item.perPeriodAmount > 0 && (
        ...
        >
          ...
          <strong style={{ color: "#0f172a" }}>
            $
            {item.perPeriodAmount.toLocaleString("en-US", {
        ...
        value={
          <input
            ...
            onChange={(e) =>
              onChange({
                ...item,
                monthlyTakeHome: parseFloat(e.target.value) || 0,
              })
            }
          />
        }
...
function StatementReview({
  ...
  const selectedCharges = result.recurringCharges.filter((_, i) =>
    selected.has(i),
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
      ...
      >
        {result.analysisNotes} Select which recurring charges to add to your
        expense tracker.
      </p>
      ...
      {result.recurringCharges.length === 0 ? (
        ...
        >
          No recurring charges detected. Try uploading 2–3 months of statements
          for better results.
        </div>
      ) : (
        <div
        ...
        <>
          ...
          >
            <span style={{ fontSize: "13px", color: "#64748b" }}>
              {selected.size} selected · monthly total
            </span>
            <span
              style={{
                fontSize: "14px",
          ...
          <button
            ...
            style={{
              ...
              border: "none",
              background:
                selected.size === 0 ? "rgba(37,99,235,0.4)" : "#2563eb",
              color: "#ffffff",
              fontSize: "14px",
              fontWeight: 600,
          ...
          >
            Add {selected.size} Expense{selected.size !== 1 ? "s" : ""} to My
            Plan
          </button>
        </>
      )}
...
export default function DocumentImportModal({
  ...
  const [incomeItem, setIncomeItem] = useState<ExtractedIncomeItem | null>(
    null,
  );
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if ((err as any)?.message?.includes("timeout")) {
        ...
        msg =
          "Something went wrong extracting your document. Please try again.";
      }
        ...
        <div style={{ padding: "20px", overflowY: "auto", flex: 1 }}>
          ...
          {step === "upload" && fileType && (
            <>
              ...
              {fileType === "statement" && (
                ...
                >
                  💡 Processing may take 30-120 seconds depending on statement
                  complexity.
                </div>
              )}
            </>
            ...
            <>
              ...
              {result.type === "debt" && !debtItem && (
                ...
                >
                  ...
                  />
                  <p>
                    No debt data could be extracted from this document. Try a
              ...
              {result.type === "income" && !incomeItem && (
                ...
                >
                  ...
                  />
                  <p>
                    No income data could be extracted. Try a pay stub or W2, or
                    enter your take-home pay manually.
                  </p>
                </div>
              )}
          ...
          {step === "done" && (
            <div style={{ textAlign: "center", padding: "32px 20px" }}>
              <CheckCircle2
                size={40}
                style={{ color: "#10b981", margin: "0 auto 14px" }}
              />
              <h3
                style={{
                  fontSize: "16px",
              ...
              >
                Your plan has been updated with the imported data. Review and
                adjust anytime.
              </p>
              <button
                type="button"
