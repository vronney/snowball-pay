const plans = [
  {
    name: 'Free', tagline: 'Start your journey', price: 0, period: 'forever',
    desc: 'Everything you need to start eliminating debt — no strings attached.',
    badge: null, featured: false, accent: '#2563eb',
    cta: 'Get Started Free', ghost: true, href: '/auth/login?returnTo=/dashboard',
    features: [
      'Track up to 5 debts',
...
const trustItems = [
  ...
  { icon: '🚫', text: 'No bank access required' },
...
export default function Pricing() {
  return (
    <section id="pricing" style={{ padding: '112px 24px', position: 'relative', overflow: 'hidden', background: '#ffffff' }}>
      ...
      <div style={{ maxWidth: '1120px', margin: '0 auto', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '72px' }}>
          ...
          <p style={{ fontSize: '17px', color: '#64748b', maxWidth: '440px', margin: '0 auto', lineHeight: 1.7 }}>
            Start free, upgrade when you&apos;re ready. Every plan comes with a 7-day trial of Pro features.
          </p>
        </div>

        {/* Pricing cards */}
        <div className="lp-pricing-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '20px', alignItems: 'start' }}>
          {plans.map((plan) => (
            <div
              key={plan.name}
            ...
            >
              ...
              {/* Badge */}
              <div style={{ height: '32px', marginBottom: '16px', display: 'flex', alignItems: 'center' }}>
                {plan.badge && (
                  <div style={{
                    display: 'inline-flex', alignItems: 'center', gap: '6px',
                    padding: '4px 12px', borderRadius: '999px',
                    fontSize: '10px', fontWeight: 800, letterSpacing: '0.07em',
                    background: plan.featured ? 'rgba(37,99,235,0.08)' : `${plan.accent}0f`,
                    border: `1px solid ${plan.featured ? 'rgba(37,99,235,0.2)' : plan.accent + '28'}`,
                    color: plan.featured ? '#2563eb' : plan.accent,
                  }}>
                    {plan.featured && <span style={{ fontSize: '9px' }}>⭐</span>}
                    {plan.badge}
                  </div>
                )}
              </div>

              <p style={{ fontSize: '20px', fontWeight: 800, color: '#0f172a', letterSpacing: '-0.02em', marginBottom: '4px' }}>{plan.name}</p>
              <p style={{ fontSize: '12px', color: '#94a3b8', fontWeight: 500, marginBottom: '22px' }}>{plan.tagline}</p>

              {/* Price */}
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px', marginBottom: '6px' }}>
                {plan.price === 0 ? (
                  <span style={{ fontSize: '38px', fontWeight: 900, color: '#0f172a', letterSpacing: '-0.04em', lineHeight: 1 }}>Free</span>
                ) : (
                  <>
                    <span style={{ fontSize: '15px', fontWeight: 700, color: '#94a3b8', alignSelf: 'flex-start', marginTop: '8px' }}>$</span>
                    <span style={{ fontSize: '42px', fontWeight: 900, color: '#0f172a', letterSpacing: '-0.04em', lineHeight: 1 }}>{plan.price}</span>
                  </>
                )}
              </div>
              <p style={{ fontSize: '12px', color: '#94a3b8', fontWeight: 500, marginBottom: '18px' }}>{plan.period}</p>

              <p style={{ fontSize: '13.5px', lineHeight: 1.65, color: '#64748b', marginBottom: '26px', minHeight: '44px' }}>{plan.desc}</p>
              ...
              <a
                ...
                style={{
                  ...
                  fontSize: '14px', padding: '13px 20px', marginBottom: '24px',
                }}
              >
                {plan.cta}
              </a>
        ...
        {/* Trust strip */}
        <div style={{ marginTop: '52px', display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '10px' }}>
