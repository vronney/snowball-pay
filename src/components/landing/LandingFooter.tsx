...
export default function LandingFooter() {
  return (
    <footer style={{ borderTop: '1px solid rgba(15,23,42,0.08)', backgroundColor: '#f1f5f9' }}>
      <div style={{ maxWidth: '1160px', margin: '0 auto', padding: '64px 24px 40px' }}>
        ...
        <div className="lp-ftr-grid" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: '48px', marginBottom: '56px' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '14px', lineHeight: 1.7, color: '#64748b', maxWidth: '280px', marginBottom: '24px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              {[
                ...
                <div key={i} title={s.title} style={{ width: '34px', height: '34px', borderRadius: '9px', background: 'rgba(15,23,42,0.05)', border: '1px solid rgba(15,23,42,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', color: '#64748b', cursor: 'pointer', transition: 'background 0.2s, color 0.2s' }}>
                  {s.label}
                </div>
              ))}
            </div>
          ...
          {Object.entries(footerLinks).map(([heading, links]) => (
            <div key={heading}>
              <p style={{ fontSize: '11px', fontWeight: 800, letterSpacing: '0.14em', color: '#0f172a', textTransform: 'uppercase', marginBottom: '20px' }}>
                {heading}
              </p>
        ...
        {/* Divider */}
        <div style={{ height: '1px', background: 'rgba(15,23,42,0.08)', marginBottom: '28px' }} />
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
            <span style={{ fontSize: '11px', color: '#64748b', fontWeight: 600 }}>🔒 SSL Secured</span>
            <span style={{ fontSize: '11px', color: '#64748b', fontWeight: 600 }}>🛡️ GDPR Compliant</span>
          </div>
        </div>
      </div>
    </footer>
