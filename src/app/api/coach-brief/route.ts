...
- "redirectAmount" must equal the total EXTRA dollars (never any minimum) your nextAction proposes moving this month. It is compared programmatically against the stated Planned acceleration — if it exceeds that ceiling, the response is rejected outright. Use 0 when nextAction does not move money between debts. The one exception is "set_acceleration": there the ceiling is your own "targetExtra" (the new extra replaces the planned one), so "redirectAmount" must be 0 or at most "targetExtra".
